...
get_data(ProcId,Key)->
  DataMap = get(ProcId),
  yymg_map:get_value(Key,DataMap).

get_data(ProcId,Key,Default)->
  DataMap = get(ProcId),
  yymg_map:get_value(Key,DataMap,Default).
...
get_all_kvlist(ProcId)->
  DataMap = get(ProcId),
  yymg_map:to_kv_list(DataMap).

put_data(ProcId,Key, Data) ->
  DataMap = get(ProcId),
  NewDataMap = yymg_map:put_value(Key, Data,DataMap),
...
delete_data(ProcId,Key) ->
  DataMap = get(ProcId),
  case yymg_map:delete(Key,DataMap) of
    {?NOT_FOUND} -> {?NOT_FOUND};
    {?OK,Value,NewDataMap} ->
      put(ProcId,NewDataMap),
      {?OK,Value}
